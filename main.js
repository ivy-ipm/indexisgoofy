document.addEventListener('DOMContentLoaded', () => {
  ...
  // NAV BUTTONS
  navHome.addEventListener('click', () => showSection('homepage'));
  navTools.addEventListener('click', () => showSection('tools'));
  navGames.addEventListener('click', () => showSection('games'));
  navLogin.addEventListener('click', () => { loginBox.style.display = 'flex'; });
    ...
    switch (tool) {
      ...
      case 'Player Lookup':
        toolBox.innerHTML = `
          ...
          <button id="playerBtn">Search</button>
  ...
  async function downloadLevel() {
    ...
    try {
      ...
      output.textContent = 'Failed to download level: ' + e.message;
    }
  }
  ...
  function playerLookup() {
    ...
    setTimeout(() => {
      output.textContent = `Levels for ${user}:\n- Level1\n- Level2\n- Level3`;
    }, 800);
  }
